...
```
          ┌────────────┐
          │    uom     │  đơn vị tính (kg, g, lít, cái, thùng)
          └─────┬──────┘
...
```
→ FG-CREAM50 = 1000 × 6.27 = **6270**; tổng giá trị kho ≈ **7460**.
...
### 7.7 Tiến độ nhận hàng theo PO
```sql
SELECT po.po_no, i.code, p.ordered_qty, p.received_qty, p.open_qty
FROM v_po_line_progress p JOIN purchase_order po ON po.id=p.po_id JOIN item i ON i.id=p.item_id
